...
//		'csv' 		=> 'application/csv'
     ...
     */
    function response($data = array(), $http_code = 200, $stopProcess = FALSE)
    {
   		if(empty($data))
    	{
    ...
    private function _format_xml($data = array(), $structure = NULL, $basenode = 'xml')
    {
		...
		return html_entity_decode($structure->asXML(), ENT_NOQUOTES, "UTF-8");
    }
    ...
    private function _format_rawxml($data = array(), $structure = NULL, $basenode = 'xml')
    {
		...
		foreach( $data as $key => $value)
		{
			...
			if (is_array($value) || is_object($value))
			{
				$node = $structure->addChild($k_format_rawxmley);
				// recrusive call.
    ...
    function getArg($name) {
        if (!isset($this->_args[$name])) return FALSE;
        return $this->_args[$name];
    }
    
    /**
     * リクエストメソッドに対応した入力値を全て返す。
     * @return array 入力値マップ
     */
    public function method_args() {
    	$args = array_merge($this->_args, $_GET);
    	if ($this->_method=='get') {
    		$args = $args;
...
//    		$args = array_merge($args, $_POST);
    		...
    		$args = array_merge($args, $this->_put_args);
    	} else if ($this->_method=='delete') {
    		$args = array_merge($args, $this->_delete_args);
    	}
    ...
    /**
     * 入力チェックを行い、エラーであれば処理を中断し、
     * エラーレスポンスを行う。
     * @param int $httpStatusCode
     */
    protected function validation_run($httpStatusCode=400) {
    	if (isset($this->validation)) {
    		if ($this->validation->run()===false) {
				apiResult($httpStatusCode, $this->validation->error_string);
			}	
    	}
    }
    
    public function get_str($key = NULL, $xss_clean = TRUE) {
    	$val = $this->get($key, $xss_clean);
    	return $val === false ? '' : $val;
    }

    public function post_str($key = NULL, $xss_clean = TRUE) {
    	$val = $this->post($key, $xss_clean);
    	return $val === false ? '' : $val;
    }

    public function put_str($key = NULL, $xss_clean = TRUE) {
    	$val = $this->put($key, $xss_clean);
    	return $val === false ? '' : $val;
    }
    
    public function delete_str($key = NULL, $xss_clean = TRUE) {
    	$val = $this->delete($key, $xss_clean);
    	return $val === false ? '' : $val;
    }
}
?>